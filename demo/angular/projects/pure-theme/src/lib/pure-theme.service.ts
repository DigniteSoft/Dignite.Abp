...
export class PureThemeService {
  ...
  setCurrentApp(event: NavigationEnd) {
    ...
    const route = this.routeService.find(
      m => m.path === url.substring(0, url.indexOf('?') != -1 ? url.indexOf('?') : url.length)
    );
  ...
  getRouteGranted(item: TreeNode<ABP.Route>): boolean {
    ...
    if (
      (!item.requiredPolicy ||
        (item.requiredPolicy && this.getGrantedPolicy(item.requiredPolicy) && item.path)) &&
